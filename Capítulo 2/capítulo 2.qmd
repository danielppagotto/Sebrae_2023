...
Para esse tratamento, além da base `empresa`, vamos precisar da base `socio`. O intuito é identificar também os nomes dos socios para definir o gênero, por meio da função `get_gender`, como foi com as microempresas.
...
# writexl::write_xlsx(evolucao_municipios, "evolucao_empresas_municipio.xlsx")
```
...
cnae_municipios <- rbind(cnae_me_municipios,
      cnae_naome_municipios) |> 
  group_by(genero, cod_IBGE, Municipio, cnae, nm_cnae) |> 
  summarise(total = sum(n))  

writexl::write_xlsx(cnae_municipios, "cnae_municipios.xlsx")
