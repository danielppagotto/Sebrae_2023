...
writexl::write_xlsx(base_completa1, "frequencia_masc_fem_todas_empresas.xlsx")
...
taxa_pop_sexo_ano <- 
    ...
    mutate(cod_IBGE = as.numeric(cod_IBGE)) |> 
    mutate(ano = as.numeric(ano)) |> 
    mutate(genero = recode(genero, "Female" = "Femenino", "Male" = "Masculino"))
    left_join(pop_total, by = c("ano"="ANO","cod_IBGE"="COD_MUN")) #|>
    #group_by(ano, Municipio, cod_IBGE, SEXO, POPULACAO) |> 
    mutate(taxa = (total/POPULACAO) * 100)


```

Em seguida, com a base de evoluções, fizemos a base de taxa de emprendedores por sexo e ano (levando em conta a população de homens e mulheres por município)

```{r}

#A taxa de empreendedores (levando em conta a população de homens e mulheres por município)
...
writexl::write_xlsx(taxa_pop_sexo_ano, "taxa_pop_sexo_ano.xlsx")
