...
# writexl::write_xlsx(teste_srv2, "resultados_pnadc_cor_raca.xlsx")
...
teste_srv3 |> 
  ggplot(aes(x = V2010, y = log(renda_media))) + geom_boxplot() + 
  theme_minimal() 
...
teste <- pnad_srvyr[["variables"]]

teste_goias <- 
  teste |> 
  filter(UF == "Goiás" ) |> 
  filter(V2007 == "Mulher") |> 
  filter(VD4007 == "Empregador" | 
         VD4007 == "Conta própria") |> 
  select(VD4016, VD4017, VD4010, V2010, V1023, ID_DOMICILIO,
         VD4036)
...
writexl::write_xlsx(teste_srv4, "resultados_perfil_empreendimentos.xlsx")
